 ...
 * auto-updating. * <br> * It is a <b>BUKKIT POLICY</b> that you include a
 * boolean value in your config that prevents the auto-updater from running
 * <b>AT ALL</b>. * <br> * If you fail to include this option in your config,
 * your plugin will be <b>REJECTED</b> when you attempt to submit it to
 * dev.bukkit.org. * <p> * An example of a good configuration option would be
 * something similar to 'auto-update: true' - if this value is set to false you
 * may NOT run the auto-updater. * <br> * If you are unsure about these rules,
 * please read the plugin submission guidelines: http://goo.gl/8iU5l
 *
 *  * @author H31IX
 */
public class Updater {
    ...
    /**
     * * Gives the dev the result of the update process. Can be obtained by
     * called getResult().
     */
    public enum UpdateResult {

        /**
         * * The updater found an update, and has readied it to be loaded the
        ...
        FAIL_DBO(4),
        /**
         * * When running the version check, the file on DBO did not contain
         * the a version in the format 'vVersion' such as 'v1.0'.
         */
        FAIL_NOVERSION(5),
        /**
         * * The slug provided by the plugin running the updater was invalid
         * and doesn't exist on DBO.
         */
        FAIL_BADSLUG(6),
        /**
         * * The updater found an update, but because of the UpdateType being
         * set to NO_DOWNLOAD, it wasn't downloaded.
         */
        UPDATE_AVAILABLE(7);
    ...
    /**
     * * Get the total bytes of the file (can only be used after running a
     * version check or a normal run).
     */
    ...
    private void unzip(String file) {
        try {
            ...
            e = null;
            zipFile.close();
            zipFile = null;            // Move any plugin data folders that were included to the right place, Bukkit won't do this for us.
    ...
    private String getFile(String link) {
        ...
        try {            // Open a connection to the page
            ...
            while ((line = buff.readLine()) != null) {
                counter++;                // Search for the download link 
    ...
    private boolean versionCheck(String title) {
        if (type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split("v").length == 2) {
                ...
                if (hasTag(version) || version.equalsIgnoreCase(remoteVersion) || curVer >= remVer) {                    // We already have the latest version, or this build is tagged for no-update
    ...
    /**
     * * Evaluate whether the version number is marked showing that it should
     * not be updated by this program
     */
    ...
    private void readFeed() {
        try {            // Set header values intial to the empty string
            String title = "";
            String link = "";            // First create a new XMLInputFactory
            XMLInputFactory inputFactory = XMLInputFactory.newInstance();            // Setup a new eventReader
            InputStream in = read();
            XMLEventReader eventReader = inputFactory.createXMLEventReader(in);            // Read the XML document
            while (eventReader.hasNext()) {
                ...
                if (event.isStartElement()) {
                    ...
                    if (event.asEndElement().getName().getLocalPart().equals(ITEM)) {                        // Store the title and link of the first entry we get - the first file on the list is all we need
                        versionTitle = title;
                        versionLink = link;                        // All done, we don't need to know about older files.
                        break;
                    }
                }
            }
